    ...
    pub fn hash(str: []const u8) Ripemd160 {
        ...
        { // compress remaining string, (str.len % 64) bytes
            ...
            while (i < (str.len & 63)) : (i += 1) {
                chunk[i >> 2] ^= (@as(u32, @intCast(ptr[0])) << @as(u5, @intCast(8 * (i & 3))));
                ptr += 1;
            }

            chunk[(str.len >> 2) & 15] ^= @as(u32, 1) << @as(u5, @intCast(8 * (str.len & 3) + 7));
            ...
            chunk[14] = @as(u32, @intCast(str.len << 3));
            chunk[15] = @as(u32, @intCast((str.len >> 29) | (0 << 3)));
            compress(&temp, chunk);
        }
        ...
        { // write final hash
            var i: usize = 0;
            while (i < 160 / 8) : (i += 4) {
                std.mem.writeInt(u32, @as(*[4]u8, @ptrCast(&res.bytes[i])), temp[i >> 2], .little);
            }
        }
...
fn compress(hash: *[5]u32, words: [16]u32) void {
    ...
    @memcpy(left[0..5], hash[0..5]);
    @memcpy(right[0..5], hash[0..5]);
...
fn round(
    ...
    inline while (i < 16) : (i += 1) {
        ...
        hash[@mod(r + 0, 5)] = std.math.rotl(u32, hash[@mod(r + 0, 5)], @as(u32, @intCast(shifts[i]))) +% hash[@mod(r + 4, 5)];
